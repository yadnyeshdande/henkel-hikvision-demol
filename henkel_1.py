...
class DrawingWidget(QLabel):
	...
	def __init__(self, parent=None):
		...
		# Colors and styling
		self.boundary_color = QColor(0, 255, 0)  # Green
	...
	def update_display(self):
		...
		for i, rect in enumerate(self.rectangles):
			...
			painter.drawText(label_pos, f"Area {i + 1}")
	...
	def get_instruction_text(self):
		...
		if completed < 3:
			return f"Draw boundary {completed + 1} of 3 - Click and drag to create rectangle"
		else:
			return "All 3 boundaries completed! Click 'Save Boundaries' to proceed."
...
class TrainingPage(QWidget):
	...
	def init_ui(self):
		...
		self.clear_btn = QPushButton(" Clear All Boundaries")
	...
	def stop_camera(self):
		...
		self.start_camera_btn.clicked.connect(self.start_camera)
		self.capture_btn.setEnabled(False)
		self.status_label.setText(" Camera stopped - Start camera to continue")
		self.status_label.setStyleSheet(
	...
	def capture_frame(self):
		...
		if hasattr(self, 'current_frame'):
			...
			self.status_label.setText(" Photo captured! Now draw 3 boundaries for blue oil containers")
			self.status_label.setStyleSheet(
	...
	def update_instruction_text(self):
		...
		self.instruction_label.setText(f" {instruction_text}")

	def clear_boundaries(self):
		"""Clear all boundaries"""
		self.drawing_widget.clear_boundaries()
		self.update_instruction_text()
		self.status_label.setText(" Boundaries cleared - Draw new boundaries")
		self.status_label.setStyleSheet(
			"padding: 15px; background-color: #FFF3E0; color: #E65100; "
			"border: 1px solid #FF9800; border-radius: 5px; font-weight: bold;"
		)
	...
	def save_boundaries(self):
		...
		if len(self.drawing_widget.boundaries) != 3:
			QMessageBox.warning(self, "Error", "Please draw exactly 3 boundaries first!")
			return
		# Save boundaries and reference frame
		data = {
			'boundaries': self.drawing_widget.boundaries,
		...
		try:
			...
			QMessageBox.information(
				self, "Success",
				" Boundaries saved to boundaries.json\n🖼️ Reference frame saved\n\nYou can now go to Detection!"
			)
...
class DetectionPage(QWidget):
	...
	def init_ui(self):
		layout = QVBoxLayout()
		# Title
		title = QLabel("Detection - Blue Oil Container Monitoring")
		title.setStyleSheet("font-size: 18px; font-weight: bold; margin: 10px;")
		title.setAlignment(Qt.AlignCenter)
		layout.addWidget(title)
		# Controls
		...
		self.status_labels = []
		for i in range(3):
			status_label = QLabel(f"Area {i + 1}: Unknown")
	...
	def load_model(self):
		...
		file_path, _ = QFileDialog.getOpenFileName(
			self, "Select YOLO Model", "", "YOLO Model Files (*.pt *.onnx *.engine);;All Files (*)"
		)
		if file_path:
			try:
				...
				QMessageBox.information(self, "Success", f"YOLO model loaded: {os.path.basename(file_path)}")
			except Exception as e:
	...
	def detect_objects(self):
		...
		try:
			...
			# Filter for blue container classes (bottles, containers, etc.)
			blue_object_classes = ['bottle', 'cup', 'bowl', 'vase', 'toilet']  # Add more as needed
			detected_objects = []
			for result in results:
				...
				if boxes is not None:
					for box in boxes:
						...
						# model.names may be on different attributes depending on ultralytics version
						class_name = None
						...
						if class_name in blue_object_classes and confidence is not None and confidence > 0.5:
			...
			# Check each boundary area
			area_status = []
			for i, boundary in enumerate(self.boundaries):
				objects_in_area = self.check_objects_in_boundary(detected_objects, boundary)
				area_status.append(len(objects_in_area) > 0)
				# Update status label
				status_text = f"Area {i + 1}: {'OK' if area_status[i] else 'Not OK'}"
				color = "#4CAF50" if area_status[i] else "#F44336"  # Green or Red
				text_color = "white"
			...
			# Draw visualization
			self.draw_detection_frame(frame, detected_objects, area_status)
			# Update overall status
			all_ok = all(area_status)
			overall_text = "Overall Status: All Areas OK" if all_ok else "Overall Status: Issues Detected"
			overall_color = "#4CAF50" if all_ok else "#F44336"
	...
	def draw_detection_frame(self, frame, detected_objects, area_status):
		"""Draw detection results on frame"""
		import time
		display_frame = frame.copy()
		# Draw boundary rectangles
		for i, boundary in enumerate(self.boundaries):
			color = (0, 255, 0) if area_status[i] else (0, 0, 255)  # Green if OK, Red if not
			cv2.rectangle(
				display_frame,
...
class MainWindow(QMainWindow):
	...
	def init_ui(self):
		self.setWindowTitle("Blue Container Detection System - PSB Automation")
		...
		# Add status bar
		self.statusBar().showMessage("Ready - Start with Training to define boundaries")
	...
	def show_about(self):
		...
		QMessageBox.about(
			self, "About",
			"Blue Container Detection System\n\n"
			"This application uses YOLO11 to detect blue containers\n"
			"and monitor their presence within defined boundaries.\n\n"
			"Features:\n"
			"• Define custom detection boundaries\n"
