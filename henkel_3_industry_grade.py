...
class DetectionThread(QThread):
    ...
    def __init__(self, model_path, boundaries):
        ...
        # Only blue box boundaries are used (3 areas)
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            # Check blue box boundaries (3 areas expected)
            blue_status = []
    ...
    def finish_rectangle(self):
        ...
        if self.start_point and self.end_point:
            ...
            if abs(x2 - x1) > 10 and abs(y2 - y1) > 10:
                ...
                # All saved boundaries are blue boxes (3 expected)
                boundary = {
                    'x1': x1 * self.scale_x,
                    'y1': y1 * self.scale_y,
                    'x2': x2 * self.scale_x,
...
class TrainingPage(QWidget):
    ...
    def init_ui(self):
        ...
        instructions = QLabel(
            "Instructions:\n"
            "1. Draw 3 boundaries for BLUE BOXES\n"
            "2. Total: 3 boundaries required"
        )
    ...
    def save_boundaries(self):
        ...
        if len(self.drawing_widget.boundaries) != 3:
            QMessageBox.warning(self, "Error", "Please draw exactly 3 boundaries!\n(3 blue boxes)")
            return
...
class DetectionPage(QWidget):
    ...
    def init_ui(self):
        ...
        # (No yellow mark status - only 3 blue box areas)
    ...
    def load_boundaries(self):
        ...
        try:
            if os.path.exists('boundaries.json'):
                ...
                logger.info(f"Boundaries loaded: {len(self.blue_box_boundaries)} blue")
            else:
    ...
    def on_detection_ready(self, result):
        ...
        try:
            ...
            # Update overall status based on blue boxes only
            all_blue_ok = all(result['blue_status'])
            overall_ok = all_blue_ok
