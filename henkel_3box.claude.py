        ...
        for name, thread in threads_to_stop:
            if thread:
                try:
                    ...
                    thread.stop()
                    if not thread.wait(3000):  # Wait max 3 seconds
...
4. Draw 3 rectangles for blue box areas
5. Click "Save Boundaries"

=== DETECTION MODE ===
1. Click "Load Model" and select your trained YOLO model (.pt file)
2. Select camera
3. Click "Start Detection"
4. Monitor status indicators in real-time
5. System automatically controls relay based on detection

=== PRODUCTION FEATURES ===
• Auto-recovery: System restarts automatically on errors
• Watchdog: Detects frozen system and recovers
• Health monitoring: Continuous system health checks
• Statistics: Track success rate and uptime
• Logging: All events logged for troubleshooting

=== TROUBLESHOOTING ===
• Camera not working: Check connections, try different camera
• Model not loading: Ensure .pt file is compatible YOLO model
• Relay not working: Check USB connection and drivers
• Low FPS: Reduce camera resolution or use faster model

=== LOGS ===
View logs via File → View Logs
Logs include timestamps, errors, and performance metrics
Old logs (30+ days) can be cleared via File → Clear Old Logs

=== SUPPORT ===
Check log files for detailed error information
Export statistics for analysis via File → Export Statistics
"""
