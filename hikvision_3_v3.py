...
class DetectionThread(QThread):
    ...
    def __init__(self, model_path, boundaries):
        ...
        # Normalize boundaries for blue oil cans
        self.blue_oil_can_boundaries = [b for b in boundaries if b.get('type') == 'blue_oil_can']
        self.running = False
        self.model = None
        ...
        # NEW: prepare containers for model class mapping and target indices
        self.model_names = {}
        self.target_class_indices = set()
    ...
    def run(self):
        ...
        try:
            ...
            # NEW: read class names from model (ultralytics provides model.names)
            try:
                # model.names may be dict or list depending on ultralytics version
                names = getattr(self.model, 'names', None)
                if names is None:
                    # attempt to get from model.model.names (older/newer API differences)
            ...
            # NEW: determine which class indices correspond to boundary target types
            self.target_class_indices.clear()
            for b in self.boundaries:
                desired = str(b.get('type', '')).strip().lower()
                if not desired:
                    continue
                found = [idx for idx, name in self.model_names.items() if name.lower() == desired]
                if found:
                    self.target_class_indices.update(found)
                else:
                    logger.warning(
                        f"Target class '{desired}' not found in loaded model classes. "
                        f"Model classes: {list(self.model_names.values())[:20]} (showing up to 20). "
                        "Detections for this target will be ignored unless using a model with that class."
                    )
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            # Detected blue oil cans in the frame
            blue_oil_cans = []
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        # NEW: Map class id to name (if available)
                        class_name = self.model_names.get(class_id, str(class_id)).lower()
                        
                        # NEW: Only treat as target if class index is in target_class_indices
                        # If no target indices were found (empty set), nothing will be considered as blue_oil_can
                        if class_id not in self.target_class_indices:
                            # skip detections that are not the configured target class
                            continue
                        
                        # If we reach here, this detection matches the desired class in the model
                        detected_obj = {
                        ...
                        blue_oil_cans.append(detected_obj)
            
            # Check boundaries for blue oil cans
            blue_status = []
            for boundary in self.blue_oil_can_boundaries:
                objects_in_area = self.check_objects_in_boundary(blue_oil_cans, boundary)
                blue_status.append(len(objects_in_area) > 0)
            
            return {
                'frame': frame,
                'blue_oil_cans': blue_oil_cans,
...
class DrawingWidget(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self.blue_oil_can_color = QColor(0, 0, 255)
    ...
    def finish_rectangle(self):
        ...
        if self.start_point and self.end_point:
            ...
            if abs(x2 - x1) > 10 and abs(y2 - y1) > 10:
                ...
                logger.info(f"Boundary added: blue_oil_can - Total: {len(self.boundaries)}")
                self.update_display()
    ...
    def update_display(self):
        ...
        for i, rect in enumerate(self.rectangles):
            color = self.blue_oil_can_color
            label = f"Blue Oil Can {i + 1}"
    ...
    def get_instruction_text(self):
        ...
        if completed < 3:
            return f"Draw Blue Oil Can boundary {completed + 1} of 3"
        else:
...
class TrainingPage(QWidget):
    ...
    def init_ui(self):
        ...
        instructions = QLabel(
            "Instructions:\n"
            "Draw 3 boundaries for BLUE OIL CANS\n"
    ...
    def save_boundaries(self):
        ...
        if len(self.drawing_widget.boundaries) != 3:
            QMessageBox.warning(self, "Error", "Please draw exactly 3 boundaries for blue oil cans!")
            return
        
        blue_oil_can_boundaries = [b for b in self.drawing_widget.boundaries if b.get('type') == 'blue_oil_can']
        
        data = {
            'blue_oil_can_boundaries': blue_oil_can_boundaries,
        ...
        try:
            ...
            logger.info(f"Boundaries saved: {len(blue_oil_can_boundaries)} blue oil cans")
            
            QMessageBox.information(
                self, "Success",
                f"✅ Boundaries saved successfully!\n"
                f"🔵 Blue oil cans: {len(blue_oil_can_boundaries)}"
            )
        except Exception as e:
...
class DetectionPage(QWidget):
    ...
    def init_ui(self):
        ...
        blue_label = QLabel("Blue Oil Can Areas:")
        ...
        for i in range(3):
            status_label = QLabel(f"Oil Can {i + 1}: --")
    ...
    def load_boundaries(self):
        ...
        try:
            if os.path.exists('boundaries.json'):
                ...
                self.blue_oil_can_boundaries = data.get('blue_oil_can_boundaries', [])
                self.yellow_mark_boundaries = data.get('yellow_mark_boundaries', [])
                self.boundaries = data.get('all_boundaries', [])
                
                logger.info(f"Boundaries loaded: {len(self.blue_oil_can_boundaries)} blue oil cans")
            else:
    ...
    def start_detection(self):
        ...
        try:
            ...
            logger.info(f"Boundaries: {len(self.blue_oil_can_boundaries)} blue oil cans")
    ...
    def on_detection_ready(self, result):
        ...
        try:
            ...
            if all_ok:
                overall_text = "✅ All Blue Oil Cans OK"
                ...
                overall_text = f"⚠️ Blue Oil Can Missing: {missing}"
                overall_color = "#F44336"
                logger.warning(f"Detection #{self.detection_count}: Missing oil cans {missing}")
    ...
    def draw_frame(self, result):
        ...
        try:
            ...
            # Draw blue oil can boundaries
            for i, boundary in enumerate(self.blue_oil_can_boundaries):
                color = (0, 255, 0) if result['blue_status'][i] else (0, 0, 255)
                cv2.rectangle(frame,
                    (int(boundary['x1']), int(boundary['y1'])),
                    (int(boundary['x2']), int(boundary['y2'])),
                    color, 2)
                cv2.putText(frame, f"Blue Oil Can {i + 1}",
                    (int(boundary['x1']), int(boundary['y1']) - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            
            # Draw detected blue oil cans
            for obj in result.get('blue_oil_cans', []):
                x1, y1, x2, y2 = obj['bbox']
                cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 2)
                cv2.putText(frame, f"OilCan: {obj['confidence']:.2f}",
                    (int(x1), int(y1) - 10),
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.statusBar().showMessage("Industrial Detection System Ready - Draw 3 Blue Oil Can Boundaries")
...
5. Draw 3 rectangles for blue oil can areas
...
"""
    ...
    def show_about(self):
        QMessageBox.about(
            ...
            "Detects blue oil cans in 3 defined areas\n"
