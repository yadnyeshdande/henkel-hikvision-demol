...
class DetectionThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Capture model class names mapping (id -> name) for robust mapping
            try:
                self.class_names = getattr(self.model, 'names', None) or {}
                logger.info(f"Model class names: {self.class_names}")
            except Exception as e:
                self.class_names = {}
                logger.warning(f"Could not read model class names: {e}")
    ...
    def process_frame(self, frame):
        try:
            # Use a lower confidence threshold: consider detections >= 0.3
            ...
            for result in results:
                ...
                if boxes is not None:
                    for box in boxes:
                        ...
                        # Only consider detections above the lowered confidence threshold (30%)
                        if confidence < 0.3:
                            continue

                        # Map by model class name where possible to avoid index mismatches
                        class_name = ''
                        try:
                        ...
                        if class_name:
                            if 'oil' in class_name:
                                oil_cans.append(detected_obj); mapped = True
                            elif 'lance' in class_name:
                                lances.append(detected_obj); mapped = True
                            elif 'bunk' in class_name or 'hole' in class_name:
                                bunk_holes.append(detected_obj); mapped = True
                            elif 'teflon' in class_name or 'teflon' in class_name.lower():
                                teflons.append(detected_obj); mapped = True

                        if not mapped:
                            # fallback to previous fixed-index mapping
                            if class_id == 0:
            ...
            # For presence categories we allow partial overlap (not strictly fully inside)
            oil_can_status = []
            for boundary in self.oil_can_boundaries:
                objects_in_area = self.check_objects_in_boundary(oil_cans, boundary, strict=False)
                oil_can_status.append(len(objects_in_area) > 0)
            
            lance_present_status = []
            for boundary in self.lance_present_boundaries:
                objects_in_area = self.check_objects_in_boundary(lances, boundary, strict=False)
                lance_present_status.append(len(objects_in_area) > 0)
            
            # For absent checks (lance absent, teflon absent) be strict: object must be inside to count
            lance_absent_status = []
            for boundary in self.lance_absent_boundaries:
                objects_in_area = self.check_objects_in_boundary(lances, boundary, strict=True)
                lance_absent_status.append(len(objects_in_area) == 0)
            
            bunk_hole_status = []
            for boundary in self.bunk_hole_boundaries:
                objects_in_area = self.check_objects_in_boundary(bunk_holes, boundary, strict=False)
                bunk_hole_status.append(len(objects_in_area) > 0)
            
            teflon_absent_status = []
            for boundary in self.teflon_absent_boundaries:
                objects_in_area = self.check_objects_in_boundary(teflons, boundary, strict=True)
    ...
    def check_objects_in_boundary(self, objects, boundary, strict=False):
        """
        objects: list of detected objects with 'bbox' and 'center' and 'confidence'
        boundary: dict containing 'polygon' (list of [x,y]) or rect keys
        strict: if True, require the object's center to be strictly inside the polygon (val > 0)
                if False, allow partial overlap (any bbox corner or center inside polygon)
        """
        objects_in_boundary = []
        try:
            # Polygon boundary (list of [x,y])
                ...
                for obj in objects:
                    ...
                    try:
                        ...
                        if strict:
                            ...
                            # non-strict: consider object if center or any bbox corner is inside/on-edge
                            ...
                            if corner_inside:
                                objects_in_boundary.append(obj)
                                continue
                            # As a last resort, approximate overlap by checking bbox bounding-rect intersection with polygon bbox
                            try:
                                poly_x = contour[:, 0]
                                poly_y = contour[:, 1]
                                px1, px2 = float(np.min(poly_x)), float(np.max(poly_x))
                                py1, py2 = float(np.min(poly_y)), float(np.max(poly_y))
                                # bbox vs polygon bbox overlap
                                if not (x2 < px1 or x1 > px2 or y2 < py1 or y1 > py2):
                                    # they overlap in bbox sense -> accept as partial overlap
                                    objects_in_boundary.append(obj)
                            except Exception:
                                pass
                    except Exception:
                        # fallback to bounding box check if polygon test fails
                        pass
            else:
                # Backwards-compatible axis-aligned rectangle format
                for obj in objects:
                    center_x, center_y = obj['center']
                    ...
                    if strict:
                        # require center strictly inside rectangle
                        if x1 < center_x < x2 and y1 < center_y < y2:
                            objects_in_boundary.append(obj)
                    else:
                        # non-strict: center or any corner inside or bbox overlap
                        if (x1 <= center_x <= x2 and y1 <= center_y <= y2):
                            objects_in_boundary.append(obj)
                            continue
                        ...
                        corner_inside = any(x1 <= cx <= x2 and y1 <= cy <= y2 for cx, cy in corners)
                        if corner_inside:
                            objects_in_boundary.append(obj)
                            continue
                        # bbox overlap check
...
class DrawingWidget(QLabel):
    def __init__(self, parent=None):
        ...
        self.end_point = None
        # polygons stores drawn polygons in widget/display coordinates: list of {'points':[QPoint,...], 'type': str}
        self.polygons = []
        # boundaries stores polygon coordinates scaled to original image coords for saving/processing
        # each boundary is a dict with keys: 'type' and 'polygon' (list of [x,y] in original image coords)
        self.boundaries = []
    ...
    def mousePressEvent(self, event):
        # Left click: start/add polygon point. Close polygon by clicking near first point.
    ...
    def mouseReleaseEvent(self, event):
        # We handle point addition on press; release is not used for polygon drawing
        pass
    ...
    def finish_rectangle(self):
        # Old rectangle finish kept for compatibility (not used by new polygon flow)
        return

    def finish_polygon(self):
        # finalize polygon created in widget coords (self.current_points)
        if not hasattr(self, 'current_points') or len(self.current_points) < 3:
            logger.info("Polygon too small, ignored")
            return

        # determine boundary type in same order as before
        ...
        # store polygon in widget/display coords for rendering
        pts_copy = list(self.current_points)
        self.polygons.append({'points': pts_copy, 'type': boundary_type})

        # scale to original image coords for processing/saving
        scaled_polygon = []
        for p in pts_copy:
            ox = p.x() * self.scale_x
            oy = p.y() * self.scale_y
    ...
    def update_display(self):
        ...
        if self.drawing and hasattr(self, 'current_points') and len(self.current_points) > 0:
            ...
            # draw small circle at first point to indicate closable
            first = pts[0]
    ...
    def clear_boundaries(self):
        # clear polygon representations and stored boundaries
        if hasattr(self, 'polygons'):
            self.polygons.clear()
        self.boundaries.clear()
        # also reset any in-progress polygon
    ...
    def undo_last_polygon(self):
        """Remove the last completed polygon and its corresponding saved boundary.
        ...
        if self.boundaries:
            # boundaries correspond 1:1 with polygons, remove last
            self.boundaries.pop()
            removed = True
...
class TrainingPage(QWidget):
    ...
    def undo_last_boundary(self):
        # remove the last drawn boundary (polygon) during training
        if hasattr(self.drawing_widget, 'undo_last_polygon'):
            ...
            # fallback: manipulate lists directly
            if self.drawing_widget.boundaries:
                self.drawing_widget.boundaries.pop()
                if hasattr(self.drawing_widget, 'polygons') and self.drawing_widget.polygons:
                    self.drawing_widget.polygons.pop()
                logger.info("Last boundary removed (fallback)")
        self.update_instruction_text()
...
class DetectionPage(QWidget):
    ...
    def load_boundaries(self):
        try:
            if os.path.exists('boundaries.json'):
                ...
                # store reference frame shape so detection frames can be resized to match
    ...
    def start_detection(self):
        ...
        try:
            ...
            camera_wait_deadline = time.time() + 10.0  # seconds (extended wait for slower cameras)
            camera_ok = False
    ...
    def on_frame_ready(self, frame):
        if self.detection_thread and self.running:
            # Ensure detection frame resolution matches the reference frame used when drawing boundaries
            try:
                if hasattr(self, 'reference_frame_shape') and self.reference_frame_shape:
                    ref_shape = self.reference_frame_shape
                    # ref_shape is (height, width, channels) typically
                    ref_h, ref_w = int(ref_shape[0]), int(ref_shape[1])
                    h, w = frame.shape[0], frame.shape[1]
                    if (h, w) != (ref_h, ref_w):
                        # resize to reference resolution (keep BGR channels)
        ...
        try:
            ...
            for i, boundary in enumerate(self.oil_can_boundaries):
                ...
                if 'polygon' in boundary and boundary.get('polygon'):
                    ...
                    x0, y0 = pts[0]
                    cv2.putText(frame, f"OC{i + 1}", (int(x0), int(y0) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.rectangle(frame,
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0))),
                        (int(boundary.get('x2', 0)), int(boundary.get('y2', 0))),
                        color, 2)
                    cv2.putText(frame, f"OC{i + 1}",
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0)) - 5),
            ...
            for i, boundary in enumerate(self.lance_present_boundaries):
                ...
                if 'polygon' in boundary and boundary.get('polygon'):
                    ...
                    cv2.polylines(frame, [pts], isClosed=True, color=color, thickness=2)
                    x0, y0 = pts[0]
                    cv2.putText(frame, f"LP{i + 1}", (int(x0), int(y0) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.rectangle(frame,
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0))),
                        (int(boundary.get('x2', 0)), int(boundary.get('y2', 0))),
            ...
            for i, boundary in enumerate(self.lance_absent_boundaries):
                ...
                if 'polygon' in boundary and boundary.get('polygon'):
                    ...
                    cv2.polylines(frame, [pts], isClosed=True, color=color, thickness=2)
                    x0, y0 = pts[0]
                    cv2.putText(frame, f"LA{i + 1}", (int(x0), int(y0) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.rectangle(frame,
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0))),
                        (int(boundary.get('x2', 0)), int(boundary.get('y2', 0))),
            ...
            for i, boundary in enumerate(self.bunk_hole_boundaries):
                ...
                if 'polygon' in boundary and boundary.get('polygon'):
                    ...
                    cv2.polylines(frame, [pts], isClosed=True, color=color, thickness=2)
                    x0, y0 = pts[0]
                    cv2.putText(frame, f"BH{i + 1}", (int(x0), int(y0) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.rectangle(frame,
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0))),
                        (int(boundary.get('x2', 0)), int(boundary.get('y2', 0))),
            ...
            for i, boundary in enumerate(self.teflon_absent_boundaries):
                ...
                if 'polygon' in boundary and boundary.get('polygon'):
                    ...
                    cv2.polylines(frame, [pts], isClosed=True, color=color, thickness=2)
                    x0, y0 = pts[0]
                    cv2.putText(frame, f"TA{i + 1}", (int(x0), int(y0) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                else:
                    cv2.rectangle(frame,
                        (int(boundary.get('x1', 0)), int(boundary.get('y1', 0))),
                        (int(boundary.get('x2', 0)), int(boundary.get('y2', 0))),
